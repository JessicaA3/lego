...
const fetchSalesStats = async (legoSetId) => {
  try {
      ...
      const response = await fetch(`https://lego-g9ugb9z8g-jessicas-projects-0966b521.vercel.app/sales/stats/${legoSetId}`);
...
if (priceSlider) {
  priceSlider.addEventListener('input', async () => {
    ...
    try {
      const response = await fetch(`https://lego-g9ugb9z8g-jessicas-projects-0966b521.vercel.app/deals/search?price=${price}&filterBy=price-asc`);
      const body = await response.json();
      ...
      currentPagination = {
        currentPage: 1,
        pageSize: parseInt(selectShow.value) || 12,
        pageCount: Math.ceil(allDeals.length / (parseInt(selectShow.value) || 12))
      };
      render();
    } catch (error) {
